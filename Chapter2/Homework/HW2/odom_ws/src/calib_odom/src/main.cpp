...
void Scan2::CalibFlagCallBack(const std_msgs::Empty &msg)
{
    ...
    for (int i = 0; i < odom_increments.size(); i++)
    {
        ...
        tmp_transform_matrix << cos(calib_pos(2)), -sin(calib_pos(2)), 0, sin(calib_pos(2)), cos(calib_pos(2)), 0, 0, 0,
            1;
...
void Scan2::scanCallBack(const sensor_msgs::LaserScan::ConstPtr &_laserScanMsg)
{
    ...
    //得到对应的里程计数据
    if (!getOdomPose(odom_pose, _laserScanMsg->header.stamp)) return;
    ...
    std::cout << "d_scan : " << d_point_scan.transpose() << std::endl;
    std::cout << "d_odom : " << d_point_odom.transpose() << std::endl;
    // 构造旋转矩阵 生成三种位姿
    ...
    std::cout << "scan_pos_cal : " << scan_pos_cal.transpose() << std::endl;
    ...
    std::cout << "odom_pos_cal : " << odom_pos_cal.transpose() << std::endl;
...
Eigen::Vector3d cal_delta_distence(Eigen::Vector3d odom_pose)
{
    ...
    // TODO:
    Eigen::Matrix3d pos_tmp = Eigen::Matrix3d::Identity();
    pos_tmp(0, 0) = cos(last_pos(2));
    pos_tmp(0, 1) = -sin(last_pos(2));
    pos_tmp(1, 0) = sin(last_pos(2));
    pos_tmp(1, 1) = cos(last_pos(2));
    d_pos = pos_tmp.inverse() * (now_pos - last_pos);
    // end of TODO:
    // last_pos = now_pos;
