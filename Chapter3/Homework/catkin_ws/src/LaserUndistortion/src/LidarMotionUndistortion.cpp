...
pcl::visualization::CloudViewer g_PointCloudView("PointCloud View1");
...
public:
    ...
    void ScanCallBack(const champion_nav_msgs::ChampionNavLaserScanPtr &scan_msg)
    {
        ...
        for (int i = 0; i < ranges.size(); i++)
        {
            ...
            // 转回到odom坐标系下
            // T_pose^odom = T_laser^odom * T_pose^laser
            pcl::PointXYZRGB pt;
     ...
     */
    void Lidar_MotionCalibration(tf::Stamped<tf::Pose> frame_base_pose, tf::Stamped<tf::Pose> frame_start_pose,
                                 ...
                                 std::vector<double> &angles, int startIndex, int &beam_number)
    {
        ...
        Eigen::Isometry3d start_base_pose;
        Eigen::Isometry3d end_start_pose;

        tf::transformTFToEigen(frame_base_pose, base_pose);
        tf::transformTFToEigen(frame_start_pose, start_pose);
        tf::transformTFToEigen(frame_end_pose, end_pose);

        double vel_x, vel_y, vel_theta;
        // start 坐标系下的 end 点位姿
        end_start_pose = start_pose.inverse() * end_pose;
        vel_x = end_start_pose.translation().x() / beam_number;
        vel_y = end_start_pose.translation().y() / beam_number;
        vel_theta = atan2(end_start_pose.rotation()(1, 0), end_start_pose.rotation()(0, 0)) / beam_number;

        double start_base_theta;
        //  base坐标系下的 start点
        start_base_pose = base_pose.inverse() * start_pose;
        start_base_theta = atan2(start_base_pose.rotation()(1, 0), start_base_pose.rotation()(0, 0));

        for (int i = startIndex; i < startIndex + beam_number; i++)
        {
            double x = ranges[i] * cos(angles[i]) + vel_x * (i - startIndex) + start_base_pose.translation().x();
            double y = ranges[i] * sin(angles[i]) + vel_y * (i - startIndex) + start_base_pose.translation().y();
            double angle = angles[i] + vel_theta * (i - startIndex) + start_base_theta;
            ranges[i] = sqrt(x * x + y * y);
            angles[i] = angle;
        }
        // end of TODO
    }
     ...
     * @param *tf_
     */
    void Lidar_Calibration(std::vector<double> &ranges, std::vector<double> &angles, ros::Time startTime,
                           ros::Time endTime, tf::TransformListener *tf_)
    {
        //统计激光束的数量
        ...
        for (int i = 0; i < beamNumber; i++)
        {
            ...
            if (mid_time - start_time > interpolation_time_duration || (i == beamNumber - 1))
            {
                ...
                Lidar_MotionCalibration(frame_base_pose, frame_start_pose, frame_mid_pose, ranges, angles, start_index,
                                        interp_count);

                //更新时间
                start_time = mid_time;
                //  应为i+1,否则两段的起点和终点会有重合
                // start_index = i;
                start_index = i + 1;
