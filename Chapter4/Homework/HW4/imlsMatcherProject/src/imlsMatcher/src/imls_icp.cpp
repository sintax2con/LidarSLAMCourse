...
IMLSICPMatcher::~IMLSICPMatcher(void)
{
    if (m_pTargetKDTree != NULL) delete m_pTargetKDTree;

    if (m_pSourceKDTree != NULL) delete m_pSourceKDTree;
}

void IMLSICPMatcher::setIterations(int _iter) { m_Iterations = _iter; }
...
bool IMLSICPMatcher::ImplicitMLSFunction(Eigen::Vector2d x, double &height)
{
    ...
    if (m_pTargetKDTree == NULL)
    {
        std::cout << "Create KD-tree in imls func" << std::endl;
        m_targetKDTreeDataBase.resize(2, m_targetPointCloud.size());
        for (int i = 0; i < m_targetPointCloud.size(); i++)
        {
            m_targetKDTreeDataBase(0, i) = m_targetPointCloud[i](0);
    ...
    //最大搜索距离为m_r
    m_pTargetKDTree->knn(
        x, nearIndices, nearDist2, searchNumber, 0,
        Nabo::NNSearchD::SORT_RESULTS | Nabo::NNSearchD::ALLOW_SELF_MATCH | Nabo::NNSearchD::TOUCH_STATISTICS, m_h);
    ...
    {
        //说明最近邻是合法的．
        if (nearDist2(i) < std::numeric_limits<double>::infinity() && std::isinf(nearDist2(i)) == false &&
    ...
    {
        double weight = std::exp(-std::abs((x - nearPoints[i]).transpose() * (x - nearPoints[i])) / (m_h * m_h));
        height_den += weight;
        height_num += weight * ((x - nearPoints[i]).dot(nearNormals[i]));
    }
    height = height_num / height_den;
    // end of TODO
 ...
 */
void IMLSICPMatcher::projSourcePtToSurface(std::vector<Eigen::Vector2d> &in_cloud,
                                           ...
                                           std::vector<Eigen::Vector2d> &out_normal)
{
    ...
    for (std::vector<Eigen::Vector2d>::iterator it = in_cloud.begin(); it != in_cloud.end();)
    {
        ...
        if (std::isinf(nearNormal(0)) || std::isinf(nearNormal(1)) || std::isnan(nearNormal(0)) ||
            std::isnan(nearNormal(1)))
        {
                                                  ...
                                                  Eigen::Matrix3d &             deltaTrans)
{
    ...
    deltaTrans << cos(theta), -sin(theta), res(0), sin(theta), cos(theta), res(1), 0, 0, 1;
    return true;
}
...
Eigen::Vector2d IMLSICPMatcher::ComputeNormal(std::vector<Eigen::Vector2d> &nearPoints)
{
    ...
    // 计算方差
    Eigen::Matrix<double, 1, 5, Eigen::RowMajor> accu = Eigen::Matrix<double, 1, 5, Eigen::RowMajor>::Zero();
    Eigen::Matrix<double, 2, 1>                  K(0.0, 0.0);
    Eigen::Matrix<double, 2, 2>                  covariance_matrix;
    ...
    std::size_t point_count = nearPoints.size();
    for (const auto &point : nearPoints)
    {
        double x = point[0] - K.x(), y = point[1] - K.y();
        accu[0] += x * x;
        accu[1] += x * y;
        accu[2] += y * y;
        accu[3] += x;
    ...
    Eigen::MatrixXd                                eigen_values = eigen_solver.eigenvalues();
    Eigen::MatrixXd                                eigen_vector = eigen_solver.eigenvectors();

    normal = eigen_vector.col(0);
    // end of TODO
 ...
 * @return
 */
bool IMLSICPMatcher::Match(Eigen::Matrix3d &finalResult, Eigen::Matrix3d &covariance)
{
    ...
    if (m_isGetNormals == false)
    {
        ...
        for (int i = 0; i < m_targetPointCloud.size(); i++)
        {
            ...
            // kdtree 搜索半径0.15m内最多20个点
            int             K = 20;
            Eigen::VectorXi indices(K);
            Eigen::VectorXd dist2(K);
            int             num = m_pTargetKDTree->knn(
                xi, indices, dist2, K, 0.0,
    ...
    std::cout << "Compute normal time: " << ms.count() << " ms\n";
    ...
    for (int i = 0; i < m_Iterations; i++)
    {
        ...
        auto t2 = std::chrono::system_clock::now();
        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1);
        std::cout << "Project cloud time: " << ms.count() << " ms\n";
        ...
        std::cout << "solve motion estimation time: " << ms2.count() << " ms\n";
        if (flag == false)
        {
            std::cout << "ICP Iterations Failed!!!!" << std::endl;
            return false;
...
bool IMLSICPMatcher::SolverFourthOrderPolynomial(Eigen::VectorXd &p_coffi, double &lambda)
{
